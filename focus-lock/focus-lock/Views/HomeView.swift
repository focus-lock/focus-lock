...
struct HomeView: View {
    ...
    private func isScheduledToday(_ rule: Rule, now: Date) -> Bool {
        ...
        // One-time rules count only when their one-time date is today.
        if rule.isOneTime {
            guard let oneTimeDate = rule.oneTimeDate else {
                return false
            }

            return Calendar.current.isDate(oneTimeDate, inSameDayAs: now)
        }

        // Recurring rules count when today is one of their selected weekdays.
        let weekday = Calendar.current.component(.weekday, from: now)
        return rule.repeatWeekdays.contains(weekday)
    }
