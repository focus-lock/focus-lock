...
We attempted to expose `APP_GROUP_IDENTIFIER` through a custom Info.plist key:

```text
FocusLockAppGroupIdentifier
```

Runtime logs showed the app was using the bundle-ID fallback instead:

```text
(bundle identifier fallback)
```

The fallback now derives:

```text
com.focuslock.focuslockapp -> group.com.focuslock.focuslockapp
com.focuslock.focuslock-app -> group.com.focuslock.focuslock-app
```
